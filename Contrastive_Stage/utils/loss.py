...
def calc_gradient_penalty_ST(D, real_data, fake_data, term=None):
    ...
    if 'real' in term:
        ...
        gradients = autograd.grad(outputs=output, inputs=real_data, grad_outputs=torch.ones(output.size()).to(device),
    ...
    if 'fake' in term:
        ...
        gradients = autograd.grad(outputs=output, inputs=fake_data, grad_outputs=torch.ones(output.size()).to(device),
                                  create_graph=True, retain_graph=True, only_inputs=True)[0]
    ...
    if 'real_motion' in term:
        ...
        output = D(input)
        gradients = autograd.grad(outputs=output, inputs=input, grad_outputs=torch.ones(output.size()).to(device),
                                  create_graph=True, retain_graph=True, only_inputs=True)[0]
