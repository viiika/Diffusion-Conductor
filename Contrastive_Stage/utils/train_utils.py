...
class PairBuilder:
    ...
    def build_pairs(self, music, motion, sampling_strategy):
        ...
        if sampling_strategy == 'easy':
            ...
            music_1 = music[:, music_start:music_end, :].type(torch.FloatTensor).to(device)
            motion_1 = motion[:, motion_start:motion_end, :].type(torch.FloatTensor).to(device)
            music_2 = music[:, music_start:music_end, :].type(torch.FloatTensor).to(device).flip(dims=[0])
            motion_2 = motion[:, motion_start:motion_end, :].type(torch.FloatTensor).to(device).flip(dims=[0])
            ...
            music_1 = music[:, music_start_1:music_end_1, :].type(torch.FloatTensor).to(device)
            motion_1 = motion[:, motion_start_1:motion_end_1, :].type(torch.FloatTensor).to(device)
            music_2 = music[:, music_start_2:music_end_2, :].type(torch.FloatTensor).to(device)
            motion_2 = motion[:, motion_start_2:motion_end_2, :].type(torch.FloatTensor).to(device)
