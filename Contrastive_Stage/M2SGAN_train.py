    ...
    for epoch in range(args.epoch_num):
        ...
        for step, (music, real_motion) in pbar:
            if real_motion.shape[0] != args.batch_size:
                continue
            music = music.type(torch.FloatTensor).to(device)
            real_motion = real_motion.type(torch.FloatTensor).to(device)
            optimizer_G.zero_grad()

            noise = torch.randn([args.batch_size, args.sample_length, 8]).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--M2SNet', default='/Users/jinbin/5340Proj/stage_1_checkpoints/M2SNet/hard/M2SNet_last.pt')#'checkpoints/M2SNet/hard/M2SNet_last.pt')
    parser.add_argument('--transfer_music_encoder', default=True)
    parser.add_argument('--train_music_encoder', default=False)
    parser.add_argument('--M2SNet_test', default='/Users/jinbin/5340Proj/stage_1_checkpoints/M2SNet/hard/M2SNet_last.pt',
